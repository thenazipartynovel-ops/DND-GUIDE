...
function mkP(raw = {}) {
  ...
  return {
    id:          p.id || (Date.now() + Math.floor(Math.random()*99999)),
...
// ══════════════════════════════════════════════════════════════
// 🔥 FIREBASE HOOK  — bulletproof sync v2
//
// Root-cause fixes:
//  • pendingSaveKey: after every save() we store a fingerprint of
//    the saved IDs; onValue compares and skips if it matches our
//    own write — no more race-prone skipCount counter.
//  • isSavingUntil: a timestamp-based guard; any onValue firing
//    within 4 s of a save is treated as our own echo and ignored,
//    preventing stale-data overwrites even if Firebase sends two
//    callbacks (reconnect, cache flush, etc.).
//  • onInitial signals back via firebaseActiveRef so the 5-second
//    fallback timer never overwrites real Firebase data.
// ══════════════════════════════════════════════════════════════
function useFirebase() {
  const [status, setStatus] = useState("connecting");
  const state          = useRef({ pRef:null, set:null, unsub:null });
  const pendingSaveKey = useRef(null);   // fingerprint of last save
  const isSavingUntil = useRef(0);       // epoch ms — ignore onValue until this time

  const makeSaveKey = (players) =>
  ...
  const connect = useCallback(async (onInitial, onUpdate, unmountedRef, firebaseActiveRef) => {
    try {
      ...
      const unsub = onValue(pRef, snap => {
        ...
        if (firstFire) {
          firstFire = false;
          // Signal fallback timer that Firebase has responded — don't overwrite
          firebaseActiveRef.current = false;
          onInitial(arr);
          setStatus("live");
          return;
        }

        // ── Echo / stale-write guard ──────────────────────────
        const now = Date.now();
        if (now < isSavingUntil.current) return; // within our save window
        if (arr && pendingSaveKey.current !== null) {
          const incomingKey = arr.map(p => p.id).sort().join(",");
          if (incomingKey === pendingSaveKey.current) {
            // This is our own echo — dismiss the pending key and skip
        ...
        // Genuine update from another device / tab
        if (arr && arr.length) onUpdate(arr);
        setStatus("live");
      }, err => {
  ...
  const save = useCallback((players) => {
    ...
    // Stamp this save so the echo can be recognised
    pendingSaveKey.current = makeSaveKey(players);
    isSavingUntil.current  = Date.now() + 4000; // 4-second grace window
    const obj = {};
    players.forEach(p => { obj[`p_${p.id || Date.now()}`] = p; });
    fbSet(pRef, obj).catch(err => {
      console.warn("[FB save]", err.message);
      pendingSaveKey.current = null;
      isSavingUntil.current  = 0;
    });
    return true;
  }, []);
...
function useConfirm() {
  ...
  const confirm = (msg, onYes, opts={}) => setCfg({ msg, onYes, title: opts.title||"Confirm", yesLabel: opts.yesLabel||"Confirm", yesVariant: opts.yesVariant||"danger" });
  const dialog = cfg ? (
    <div onClick={() => setCfg(null)} style={{
      position:"fixed",inset:0,zIndex:2000,
      background:"#00000099",backdropFilter:"blur(12px)",
      ...
      <div onClick={e=>e.stopPropagation()} style={{
        ...
        border:`1px solid ${N.border}`,borderRadius:22,
        boxShadow:`-12px -12px 30px ${N.light}88,12px 12px 30px ${N.dark},0 0 60px #00000088`,
        padding:"28px 26px 22px",textAlign:"center",
        animation:"mPop .25s cubic-bezier(.34,1.4,.64,1) both",
      }}>
        <div style={{fontSize:32,marginBottom:12}}>{cfg.yesVariant==="danger"?"⚠️":"❓"}</div>
        <div style={{fontFamily:"'Cinzel Decorative',serif",fontSize:14,color:N.gold,marginBottom:12,letterSpacing:.3}}>{cfg.title}</div>
        <div style={{fontFamily:"'Crimson Pro',serif",fontSize:15,color:N.text,lineHeight:1.6,marginBottom:22,whiteSpace:"pre-line"}}>{cfg.msg}</div>
        <div style={{display:"flex",gap:10,justifyContent:"center"}}>
          <FBtn v="ghost" onClick={() => setCfg(null)}>Cancel</FBtn>
          <FBtn v={cfg.yesVariant} onClick={() => { cfg.onYes(); setCfg(null); }}>{cfg.yesLabel}</FBtn>
        </div>
      </div>
    </div>
  ) : null;
...
// SVG COMPONENTS
// ══════════════════════════════════════════════════════════════
const _coinGidCounter = { n: 0 };
const CoinSvg = memo(function CoinSvg({type, size=22}) {
  const c = COINS[type];
  // Unique gradient ID per instance prevents SVG gradient collisions when
  // multiple CoinSvg of the same type appear on the same page simultaneously.
  const gid = useRef(`cg_${type}_${++_coinGidCounter.n}`).current;
  return (
  ...
  return (
    <div style={{height:9,...nI(3),borderRadius:10,overflow:"hidden",position:"relative"}}>
      <div style={{
        ...
        animation: crit ? "critB 1.3s ease-in-out infinite" : "none",
        position:"relative",
      }}>
...
function DeathSaves({saves, onChange}) {
  ...
  const dot = (filled, col, onClick) => (
    <button type="button" onClick={onChange ? onClick : undefined} style={{
        ...
        <div
          ...
          <div style={{
            ...
            <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:"3px 10px"}}>
              {["1P = 10G","10G = 1P","1G = 10S","1S = 10G","1G = 100C","100C = 1G","1P = 100S","1P = 1,000C"].map(r=>(
                <div key={r} style={{
...
function WalletRow({currency}) {
  const safe = mkCurrency(currency); // guards null/undefined/partial currency objects
  const hasSome = COIN_KEYS.some(k => safe[k] > 0);
  if (!hasSome) return (
  ...
  return (
    <div style={{display:"flex",flexWrap:"wrap",gap:5,overflowX:"auto",WebkitOverflowScrolling:"touch",paddingBottom:2}}>
      {COIN_KEYS.map(k => safe[k] > 0 ? <CoinBadge key={k} type={k} amount={safe[k]}/> : null)}
    </div>
  );
}
...
const PlayerCard = memo(function PlayerCard({player:p, isDM, onEdit}) {
  ...
  const hpPct  = maxHP > 0 ? Math.min(100, Math.max(0, Math.round(p.hp/maxHP*100))) : 0;
  const col    = p.color;
          ...
          {p.inventory?.length > 0 ? (
            <div style={{display:"flex",flexWrap:"wrap",gap:4,maxHeight:72,overflowY:"auto"}}>
              {p.inventory.map((item,i) => (
                <span key={`${item}-${i}`} style={{fontSize:11,fontFamily:"'Crimson Pro',serif",...nR(2),borderRadius:5,padding:"2px 8px",color:"#c4b5fd",border:"1px solid #3a3060",flexShrink:0}}>
                  {item}
                </span>
              ))}
            </div>
...
function Modal({onClose, children, maxW=660, title}) {
  useEffect(() => {
    ...
    // Use a counter on body so nested/rapid modals don't prematurely restore scroll
    const key = "__wt_modal_depth__";
    document.body[key] = (document.body[key] || 0) + 1;
    document.body.style.overflow = "hidden";
    return () => {
      window.removeEventListener("keydown", esc);
      document.body[key] = Math.max(0, (document.body[key] || 1) - 1);
      if (document.body[key] === 0) document.body.style.overflow = "";
    };
  }, [onClose]);
    ...
    <div
      ...
      <div
        ...
        <button type="button" onClick={onClose} style={{
          ...
          background:"#ffffff0c",border:`1px solid ${N.border}`,
          color:N.dim,fontSize:16,cursor:"pointer",
          display:"flex",alignItems:"center",justifyContent:"center",
          transition:"all .15s",
        }}
          onMouseEnter={e=>{e.currentTarget.style.background="#f8717122";e.currentTarget.style.color="#f87171";}}
          onMouseLeave={e=>{e.currentTarget.style.background="#ffffff0c";e.currentTarget.style.color=N.dim;}}
  ...
  return (
    <div style={{marginBottom:14}}>
      ...
      {rows
        ...
        : <input type={type==="number"?"text":type} value={displayVal} {...(type==="number" ? {inputMode:"numeric"} : {})}
...
function FBtn({children, onClick, v="primary", style:s={}, disabled=false}) {
  ...
  const T = {
    ...
    ghost:   {bg:"transparent",hbg:N.surface,b:N.border,tc:N.dim},
  ...
  return (
    <button type="button" onClick={onClick} disabled={disabled}
      ...
      style={{
        ...
        color: disabled ? "#444" : (T.tc || N.text),
...
function StatEditor({stats, onChange}) {
  ...
  return (
    <div style={{...nI(4),borderRadius:14,padding:14,overflowX:"auto"}}>
      <div className="stat-grid" style={{display:"grid",gridTemplateColumns:"repeat(5,minmax(58px,1fr))",gap:8,minWidth:310}}>
            ...
            <div key={k} style={{textAlign:"center"}}>
              ...
              <div style={{display:"flex",gap:4,justifyContent:"center"}}>
                {[[-1,"#f87171","#2d0a0a"],[+1,"#4ade80","#0a2d14"]].map(([d,c,bg])=>(
                  <button type="button" key={d} onClick={()=>adjBonus(k,d)}
...
function CurrencyEditor({currency, onChange}) {
  // Local display buffer so typing mid-number doesn't snap value (e.g. typing "10" goes 1→10)
  const [disp, setDisp] = useState(() =>
    COIN_KEYS.reduce((a,k) => ({...a, [k]: String(currency[k]??0)}), {})
  );
  // Keep display in sync when currency changes externally (e.g. long rest, remote update)
  const prevCur = useRef(currency);
  useEffect(() => {
    if (prevCur.current === currency) return;
    prevCur.current = currency;
    setDisp(COIN_KEYS.reduce((a,k) => {
      const ext = String(currency[k]??0);
      return {...a, [k]: ext};
    }, {}));
  }, [currency]);

  const commit = (k, raw) => {
    const v = Math.max(0, safeInt(raw, 0));
    setDisp(d => ({...d, [k]: String(v)}));
    onChange({...currency, [k]: v});
  };
  const adj = (k,n) => {
    const v = Math.max(0, (currency[k]||0)+n);
    setDisp(d => ({...d, [k]: String(v)}));
    onChange({...currency, [k]: v});
  };
    ...
    <div style={{...nI(5),borderRadius:14,padding:16}}>
        ...
        {COIN_KEYS.map(k => {
          ...
          return (
            <div key={k} style={{textAlign:"center"}}>
              ...
              <input type="text" inputMode="numeric" value={disp[k]} min={0}
                onChange={e => setDisp(d => ({...d, [k]: e.target.value}))}
                onBlur={e => { commit(k, e.target.value); e.target.style.borderColor=`${coin.col}33`; }}
                ...
                onFocus={e=>{e.target.style.borderColor=coin.col;}}/>
              <div style={{display:"flex",gap:3,justifyContent:"center",marginTop:5}}>
                {[-5,-1,1,5].map(n => (
                  <button type="button" key={n} onClick={() => adj(k,n)}
      ...
      <div style={{...nR(3),borderRadius:10,padding:10,marginBottom:10}}>
        ...
        <div style={{display:"grid",gridTemplateColumns:"1fr 1fr 1fr",gap:"3px 12px",fontFamily:"'Crimson Pro',serif",fontSize:12}}>
          {["1P = 10G","1P = 100S","1P = 1,000C","1G = 10S","1G = 100C","1S = 10C"].map(r=>(
            <div key={r} style={{color:N.dim,textAlign:"center",padding:"2px 0",borderBottom:`1px solid ${N.border}44`}}>{r}</div>
          ))}
        </div>
      </div>
...
function EditModal({player, onSave, onDelete, onClose}) {
  ...
  // Sync if the underlying player data changes while modal is open
  // (e.g. remote Firebase update arrives — we keep local edits by only
  //  syncing fields the user hasn't touched; simplest safe approach:
  //  only sync if the player ID changes, meaning a different record)
  const prevId = useRef(player.id);
  useEffect(() => {
    if (prevId.current !== player.id) {
      prevId.current = player.id;
      setF({ ...player, inventoryText: (player.inventory||[]).join(", ") });
    }
  }, [player]);
  ...
  const tglCond = c => {
    const cur = Array.isArray(f.conditions) ? f.conditions : [];
    u("conditions", cur.includes(c) ? cur.filter(x=>x!==c) : [...cur,c]);
  };

  const maxHP  = effMaxHp(f);
  const hpPct  = maxHP > 0 ? Math.min(100, Math.round(safeInt(f.hp,0)/maxHP*100)) : 0;
  ...
  return (
    <Modal onClose={onClose} maxW={700} title={`⚔️ ${f.name||"Character"}`}>
      ...
      <div style={{...nR(4),borderRadius:14,padding:"12px 16px",marginBottom:16,border:`1px solid ${N.gold}18`}}>
        ...
        <div style={{display:"flex",gap:5,overflowX:"auto",paddingBottom:2,WebkitOverflowScrolling:"touch"}}>
          {[-20,-10,-5,-1,1,5,10,20].map(n => {
            ...
            return (
              <button type="button" key={n} onClick={() => adjHp(n)}
      ...
      <div style={{display:"grid",gridTemplateColumns:"repeat(auto-fill,minmax(min(260px,100%),1fr))",gap:"0 20px"}}>
        ...
        <FInp label="Current HP" value={f.hp} onChange={v=>u("hp",clamp(v,0,effMaxHp(f)))} type="number" min={0} max={effMaxHp(f)}/>
        ...
        <div style={{marginBottom:14}}>
          ...
          <input type="color" value={f.color} onChange={e=>u("color",e.target.value)}
            aria-label="Character card colour" title="Pick character colour"
      ...
      <div style={{marginBottom:16}}>
        <button type="button" onClick={()=>u("inspiration",!f.inspiration)} style={{
      ...
      <div style={{marginBottom:14}}>
        ...
        <div style={{display:"flex",flexWrap:"wrap",gap:5,maxHeight:110,overflowY:"auto",paddingRight:2}}>
          {CONDS.map(c => {
            ...
            return (
              <button type="button" key={c} onClick={() => tglCond(c)}
      ...
      <div style={{
        ...
        <FBtn v="danger" style={{fontSize:11}} onClick={() => {
          onDelete(player.id); // EditModal's parent (App) wraps deletePlayer in confirm
        }}>
...
function AddModal({onSave, onClose}) {
  ...
  const save = () => {
    ...
    const inventory = f.inventoryText.split(",").map(s=>s.trim()).filter(Boolean);
    onSave(mkP({...f, id: Date.now() + Math.floor(Math.random()*9999), inventory}));
  };
  ...
  return (
    <Modal onClose={onClose} maxW={620} title="✨ New Adventurer">
      <div style={{display:"grid",gridTemplateColumns:"repeat(auto-fill,minmax(min(240px,100%),1fr))",gap:"0 20px"}}>
        ...
        <FSel label="Status" value={f.status} onChange={v=>u("status",v)} opts={[{v:"alive",l:"✅ Alive"},{v:"unconscious",l:"💛 Unconscious"},{v:"dead",l:"☠️ Dead"}]}/>
        <FInp label="Armor Class" value={f.ac} onChange={v=>u("ac",v)} type="number" min={1} max={30}/>
        <FInp label="Starting HP" value={f.hp} onChange={v=>u("hp",Math.min(v, f.maxHp?.base||999))} type="number" min={0} max={f.maxHp?.base||999}/>
        <FInp label="Max HP (base)" value={f.maxHp?.base||20} onChange={v=>{const b=clamp(v,1,999);u("maxHp",{...f.maxHp,base:b});if(f.hp>b)u("hp",b);}} type="number" min={1} max={999}/>
        <FInp label="Initiative" value={f.initiative} onChange={v=>u("initiative",v)} type="number" min={-10} max={20}/>
        <div style={{marginBottom:14}}>
          <FL>Accent Color</FL>
          <input type="color" value={f.color} onChange={e=>u("color",e.target.value)}
...
function LoginModal({onClose, onSuccess}) {
  ...
  useEffect(() => {
    if (locked && cd === 0) {
      ...
      setTries(0);
      setErr("");
      // Refocus input after lockout expires
  ...
  const attempt = async () => {
    ...
    setErr(""); // clear stale error on new attempt
    setBusy(true);
    let ok = false;
    try {
      ok = await _verify(pw);
    } finally {
      setBusy(false); // always clears even if _verify throws
    }
  ...
  return (
    <Modal onClose={onClose} maxW={420}>
      ...
      {locked ? (
        ...
        <>
          <div style={{marginBottom:16,animation:shake?"shake .4s ease":"none"}}>
...
const PartyStats = memo(function PartyStats({players}) {
  ...
  const tHp    = players.reduce((s,p) => s + Math.max(0, safeInt(p.hp,0)), 0);
  const tMax   = players.reduce((s,p) => s + Math.max(1, effMaxHp(p)), 0);
    ...
    <div className="party-stats-bar" style={{borderBottom:`1px solid ${N.border}44`, background:`linear-gradient(90deg,${N.base}88,transparent)`}}>
      {items.map((s,i) => (
        <div key={i} style={{
          ...
          <span style={{display:"flex",alignItems:"center",fontSize:typeof s.icon==="string"?12:16,flexShrink:0}}>{s.icon}</span>
          <div>
...
// ── Session Log ───────────────────────────────────────────────
const SessionLog = memo(function SessionLog({log, show, onToggle, onClear}) {
  const ref = useRef();
  useEffect(() => {
    ...
    <div style={{
      ...
      <button type="button" onClick={onToggle} style={{
        ...
        <span style={{fontFamily:"'Cinzel',serif",fontSize:9,color:N.dim,letterSpacing:2,textTransform:"uppercase",flexShrink:0}}>
          Session Log {log.length > 0 && <span style={{color:"#4a4470",fontSize:8}}>({log.length})</span>}
        </span>
        <span style={{
        ...
        {log.length > 0 && onClear && (
          <button type="button" onClick={e=>{e.stopPropagation();onClear();}} style={{
            marginLeft:"auto",flexShrink:0,background:"transparent",border:"none",
            color:"#383060",fontSize:10,cursor:"pointer",fontFamily:"'Cinzel',serif",
            letterSpacing:.3,padding:"2px 6px",borderRadius:5,
          }} title="Clear session log">Clear</button>
        )}
        {!show && latest && (
          <span style={{
...
function ConBadge({status}) {
  ...
  return (
    <div style={{display:"flex",alignItems:"center",gap:6,padding:"4px 10px",...nR(3),borderRadius:8,border:`1px solid ${m.col}33`}}>
      <div style={{width:6,height:6,borderRadius:"50%",background:m.col,boxShadow:`0 0 6px ${m.col}`,animation:status==="live"?"livePulse 2s ease-in-out infinite":status==="connecting"?"critB 1s ease-in-out infinite":"none"}}/>
...
function App() {
  ...
  // If editingId is set but player no longer exists (deleted), auto-clear the modal
  useEffect(() => {
  ...
  // Alias so existing code that calls toast(...) still works
  ...
  useEffect(() => {
    ...
    // firebaseActiveRef starts true; set false by onInitial (Firebase responded)
    // or by the fallback timer (Firebase timed out).
    const firebaseActiveRef = { current: true };

    // onInitial: first snapshot from Firebase (sets firebaseActiveRef=false
    // so the 5-second fallback timer never overwrites real Firebase data)
    const onInitial = (firePlayers) => {
      if (unmountedRef.current) return;
      firebaseActiveRef.current = false; // CRITICAL: block fallback timer
      if (firePlayers && firePlayers.length > 0) {
        setPlayers(firePlayers);
        // Keep localStorage in sync so offline fallback stays fresh
        try { localStorage.setItem("warTable-v5", JSON.stringify(firePlayers)); } catch(_) {}
      } else {
        // Firebase empty — check localStorage before pushing hardcoded defaults
        let seed = DEF_PLAYERS;
        try {
          const raw = localStorage.getItem("warTable-v5");
          if (raw) { const parsed = JSON.parse(raw).map(mkP); if (parsed.length) seed = parsed; }
        } catch(_) {}
        setPlayers(seed);
        fbSave(seed);
        try { localStorage.setItem("warTable-v5", JSON.stringify(seed)); } catch(_) {}
      }
      setLoaded(true);
    };

    // onUpdate: genuine remote change from another device/tab
    ...
    // Fallback: if Firebase has NOT responded within 5 s, go offline
    // (firebaseActiveRef is set false by onInitial if Firebase responded early)
    const fallback = setTimeout(() => {
      if (!firebaseActiveRef.current || unmountedRef.current) return;
      firebaseActiveRef.current = false;
      try {
        const raw = localStorage.getItem("warTable-v5");
        const players = raw ? JSON.parse(raw).map(mkP) : DEF_PLAYERS;
        if (!unmountedRef.current) setPlayers(players);
      } catch {
  ...
  // Persist to Firebase AND localStorage atomically
  const persist = useCallback((newPlayers) => {
    // localStorage first — synchronous, always succeeds
    try { localStorage.setItem("warTable-v5", JSON.stringify(newPlayers)); } catch(_) {}
    fbSave(newPlayers);
  }, [fbSave]);

  const savePlayer = useCallback((upd) => {
    // Functional update avoids stale-closure bug when Firebase sync
    // updates `players` between the time the modal opened and Save is pressed.
    setPlayers(prev => {
      const old = prev.find(p => p.id === upd.id);
      const next = prev.map(p => p.id === upd.id ? upd : p);
      persist(next);
      if (old) {
        const ch = [];
    ...
    setEditingId(null);
  }, [persist, addLog, toast]);

  const deletePlayer = useCallback((id) => {
    setEditingId(null); // close modal immediately
    setPlayers(prev => {
      const p = prev.find(x => x.id === id);
      const next = prev.filter(x => x.id !== id);
      persist(next);
      addLog(`${p?.name || "Character"} removed from party`);
      toast(`${p?.name || "Character"} removed`, "danger");
      return next;
    });
  }, [persist, addLog, toast]);
  ...
  const longRest = useCallback(() => {
    confirm(
      "HP restored to max\nMost conditions cleared\nDeath saves reset\nStatus set to Alive\n\n☠️ Fallen characters unaffected.",
      () => {
        // Functional update so we always operate on the freshest state
        setPlayers(prev => {
        ...
        setPlayers(ps => {
          const healed = ps.filter(p => p.status !== "dead" && p.hp < effMaxHp(p)).length;
          addLog(`🌙 Long rest — ${healed} character${healed!==1?"s":""} fully restored`);
          toast(`Long rest complete! ${healed > 0 ? `${healed} restored.` : "All already at full HP."}`, "gold");
          return ps; // no-op read
        });
      },
      { title:"Grant a Long Rest?", yesLabel:"🌙 Rest", yesVariant:"gold" }
    );
  }, [confirm, persist, addLog, toast]);
  ...
  const sorted = useMemo(() =>
    [...players]
      ...
      .sort((a,b) => {
        switch(sortBy) {
          case "hp": {
            const aMax = effMaxHp(a) || 1;
            const bMax = effMaxHp(b) || 1;
            return (a.hp/aMax) - (b.hp/bMax);
          }
          case "name":   return a.name.localeCompare(b.name);
          case "level":  return b.level - a.level;
          case "status": return ["alive","unconscious","dead"].indexOf(a.status) - ["alive","unconscious","dead"].indexOf(b.status);
          case "wealth": {
            const aw = toCopper(a.currency) || 0;
            const bw = toCopper(b.currency) || 0;
            return bw - aw;
          }
          default:       return 0;
        }
  ...
  return (
    <>
      <style>{`
        ...
        * { scrollbar-width:thin; scrollbar-color:${N.border} transparent; }
        ...
        @media (max-width:480px) {
          ...
          .modal-inner { padding:12px 12px 16px !important; }
        }
        @media (max-width:600px) {
          ...
          /* stat editor: show 3 columns then wrap on mobile */
      ...
      <div style={{minHeight:"100vh",display:"flex",flexDirection:"column",position:"relative",zIndex:1}}>
            ...
            <div className="hdr-actions">
              ...
              {isDM ? (
                <>
                  ...
                  <FBtn v="danger"  style={{fontSize:10,padding:"7px 12px"}} onClick={() => {
                    confirm(
                      "This will permanently delete ALL characters from Firebase and localStorage.\nThis cannot be undone.",
                      () => {
                        setPlayers([]);
                        try { localStorage.removeItem("warTable-v5"); } catch(_) {}
                        persist([]);
                        addLog("⚠️ All data wiped by DM");
                        toast("All data wiped. Add new characters with + Add.", "danger");
                      },
                      { title:"⚠️ Wipe ALL Data?", yesLabel:"🗑 Wipe Everything", yesVariant:"danger" }
                    );
                  }} title="Wipe all characters (cannot undo)">🗑 Reset</FBtn>
                  <FBtn v="ghost"  style={{fontSize:10,padding:"7px 12px"}} onClick={() => { setIsDM(false); addLog("DM logged out"); toast("DM session ended","danger"); }}>🔓 Exit</FBtn>
                </>
              ) : (
        ...
        <SessionLog log={evtLog} show={logOpen} onToggle={() => setLogOpen(s=>!s)} onClear={() => setEvtLog([])}/>
      </div>
      ...
      {editing && (
        <EditModal
          ...
          onSave={savePlayer}
          onDelete={id => confirm(
            `Remove "${editing?.name}" from the party?\nThis cannot be undone.`,
            () => deletePlayer(id),
            { title:"Remove Character?", yesLabel:"🗑 Remove", yesVariant:"danger" }
          )}
